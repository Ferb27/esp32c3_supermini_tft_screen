...
## 6. Nhật ký phiên làm việc (Session Logs)

### Bản v12 (No Anime) & BongoClient v7
* **BongoClient (Python)**: Được tạo ra để lắng nghe bàn phím máy tính thông qua low-level hook (`WH_KEYBOARD_LL` bằng `ctypes`) để đếm số phím. App chạy ngầm ở System Tray (`pystray`). 
* **Khắc phục mất phím & nhiễu USB**: 
  * App Python sử dụng `Thread-safe Counter` thay vì biến cờ (boolean flag) để bắt được mọi phím kể cả khi người dùng gõ cực nhanh (VD: spam 14 phím/giây).
  * Protocol giao tiếp Serial được đổi từ ký tự đơn (`L`, `R`) sang chuỗi có tiền tố và ký tự kết thúc: `K:L\n`, `K:R\n`. Điều này loại bỏ hoàn toàn hiện tượng nhiễu USB ngẫu nhiên sinh ra ký tự rác (Ghost streaks).
  * Do đã có protocol bảo vệ, **debounce thời gian (delay 5-30ms) trên ESP32 đã được gỡ bỏ hoàn toàn** để đảm bảo không một phím nào bị drop khi gửi hàng loạt từ PC.
* **Giao diện Bongo Cat**: Mọi nỗ lực dùng mảng hình ảnh `drawRGBBitmap` từ bongo.cat đều bị lỗi layer/trong suốt. Do đó, hàm `updateBongoAnimation` trong `ui_renderer.cpp` đã được viết lại toàn bộ bằng **Flat Design Primitives** (Hình khối hình học cơ bản). Thiết kế này tái tạo chính xác 100% hình dáng meme gốc (trắng tròn, tai nhọn viền đen, bàn phím xám phẳng) mà **KHÔNG tốn chút bộ nhớ Flash nào** lưu ảnh.
