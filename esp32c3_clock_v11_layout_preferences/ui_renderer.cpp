...
void UIRenderer::updateBongoAnimation(int frameState) {
  // frameState: 0=Idle, 1=Left, 2=Right, 3=Both
  
  // Clear vùng vẽ mèo
  _tft.fillRect(10, 15, 220, 125, 0x0000);
  
  // === BÀN PHÍM TILT 3D Xéo ===
  _tft.fillTriangle(18, 142, 222, 142, 182, 118, 0x0000);
  _tft.fillTriangle(18, 142, 182, 118, 58, 118, 0x0000);
  _tft.fillTriangle(20, 140, 220, 140, 180, 120, 0xC618);
  _tft.fillTriangle(20, 140, 180, 120, 60, 120, 0xC618);
  // Lưới phím
  _tft.drawLine(40, 130, 200, 130, 0x8410);
  _tft.drawLine(70, 120, 50, 140, 0x8410);
  _tft.drawLine(105, 120, 95, 140, 0x8410);
  _tft.drawLine(140, 120, 140, 140, 0x8410);
  _tft.drawLine(175, 120, 185, 140, 0x8410);

  // === THÂN MÈO (Góc 3/4) ===
  // Tai trái (Xa)
  _tft.fillTriangle(100, 20, 80, 50, 110, 45, 0x0000);
  _tft.fillTriangle(98, 26, 84, 49, 107, 45, 0xFFFF);
  
  // Tai phải (Gần)
  _tft.fillTriangle(185, 30, 155, 60, 195, 60, 0x0000);
  _tft.fillTriangle(183, 36, 161, 58, 191, 58, 0xFFFF);

  // Thân hình bầu dục
  _tft.fillRoundRect(80, 40, 130, 100, 50, 0x0000);
  _tft.fillRect(80, 80, 130, 60, 0x0000); 
  _tft.fillRoundRect(82, 42, 126, 96, 48, 0xFFFF);
  _tft.fillRect(82, 80, 126, 58, 0xFFFF);
  
  // === MẶT MÈO ===
  // Mắt đen
  _tft.fillCircle(115, 65, 5, 0x0000); 
  _tft.fillCircle(160, 72, 6, 0x0000); 
  
  // Miệng W chéo
  int mx = 138, my = 76;
  _tft.drawLine(mx-6, my-4, mx-3, my, 0x0000);
  _tft.drawLine(mx-3, my, mx, my-3, 0x0000);
  _tft.drawLine(mx, my-3, mx+3, my, 0x0000);
  _tft.drawLine(mx+3, my, mx+6, my-4, 0x0000);
  _tft.drawLine(mx-6, my-5, mx-3, my-1, 0x0000);
  _tft.drawLine(mx-3, my-1, mx, my-4, 0x0000);
  _tft.drawLine(mx, my-4, mx+3, my-1, 0x0000);
  _tft.drawLine(mx+3, my-1, mx+6, my-5, 0x0000);
  
  // === HAI TAY MÈO ===
  bool leftDown = (frameState == 1 || frameState == 3);
  bool rightDown = (frameState == 2 || frameState == 3);
  
  // TAY TRÁI (Tay dài vươn ra)
  if (leftDown) {
    drawThickLine(_tft, 90, 85, 45, 125, 16, 0x0000); 
    drawThickLine(_tft, 90, 85, 45, 125, 14, 0xFFFF); 
  } else {
    // Vươn lơ lửng, khoe đệm thịt
    drawThickLine(_tft, 90, 85, 40, 100, 16, 0x0000);
    drawThickLine(_tft, 90, 85, 40, 100, 14, 0xFFFF);
    _tft.fillCircle(38, 100, 5, 0xF81F);
    _tft.fillCircle(31, 93, 3, 0xF81F);
    _tft.fillCircle(39, 89, 3, 0xF81F);
    _tft.fillCircle(47, 93, 3, 0xF81F);
  }
  
  // TAY PHẢI (Tay gần)
  if (rightDown) {
    drawThickLine(_tft, 175, 95, 150, 130, 16, 0x0000);
    drawThickLine(_tft, 175, 95, 150, 130, 14, 0xFFFF);
  } else {
    // Nhấc bổng lên cao như meme
    drawThickLine(_tft, 175, 95, 205, 75, 18, 0x0000);
    drawThickLine(_tft, 175, 95, 205, 75, 16, 0xFFFF);
    _tft.fillCircle(208, 72, 6, 0xF81F);
    _tft.fillCircle(198, 65, 3, 0xF81F);
    _tft.fillCircle(207, 59, 3, 0xF81F);
    _tft.fillCircle(216, 63, 3, 0xF81F);
  }
}
