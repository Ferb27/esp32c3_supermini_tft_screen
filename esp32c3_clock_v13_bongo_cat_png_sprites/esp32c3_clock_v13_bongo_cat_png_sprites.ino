...
void loop() {
  ...
  // === CẬP NHẬT HOẠT ẢNH CHO LAYOUT ANIME ĐÃ BỊ XÓA ĐỂ TIẾT KIỆM BỘ NHỚ ===
  ...
  // === BONGO CAT SERIAL HOOK (Protocol: "K:L\n", "K:R\n") ===
  static unsigned long lastBongoHit = 0;
  static bool bongoIsDown = false;
  static unsigned long lastSerialKeystroke = 0; // Debounce Serial input
  static String serialBuffer = ""; // Buffer để đọc lệnh đầy đủ
  
  while (Serial.available() > 0) {
    char c = Serial.read();
    if (c == '\n' || c == '\r') {
      // Xử lý lệnh khi nhận được ký tự kết thúc dòng
      serialBuffer.trim();
      if (serialBuffer.length() >= 3 && serialBuffer.startsWith("K:")) {
        ...
        if (key == 'L' || key == 'R' || key == 'B') {
          ...
          // Save periodically to avoid flash wear
      ...
      if (serialBuffer.length() < 10) { // Giới hạn buffer để tránh tràn bộ nhớ
        serialBuffer += c;
      }
    }
  }
  ...
  if (bongoIsDown && sysState.currentLayout == LAYOUT_BONGO_CAT) {
    if (currentMillis - lastBongoHit >= 150) {
      renderer.updateBongoAnimation(0); // Idle
      bongoIsDown = false;
    }
  }

  // === CẬP NHẬT LỊCH SỬ GÕ PHÍM KHI QUA NGÀY MỚI ===
  static int lastDay = -1;
  if (lastDay == -1 && currentTime.day > 0) {
    lastDay = currentTime.day; // Khởi tạo
  }
