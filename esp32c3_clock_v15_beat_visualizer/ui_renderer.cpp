...
void UIRenderer::updateBongoAnimation(int frameState, int audioAmp) {
  ...
  if (!canvas) {
    canvas = new GFXcanvas16(240, 115); // Increased height to allow taller bars
  }
  
  Adafruit_GFX* gfx = canvas ? (Adafruit_GFX*)canvas : (Adafruit_GFX*)&_tft;
  int offsetY = canvas ? 32 : 0;
  
  if (canvas) {
    canvas->fillScreen(0x0000);
  } else {
    _tft.fillRect(0, 32, 240, 115, 0x0000);
  }
  
  int tableY = 125 - offsetY;
  
  // === 0. DRAW EQUALIZER BARS (Behind the cat) ===
  if (audioAmp > 0) {
    int numBars = 10;
    int startX = 14;  // Wider bars, smaller startX
    int barW = 16;    // Increased width
    int gap = 6;      // Adjusted gap
    
    // Make bars much taller and more sensitive by mapping 0-200 to 0-95
    ...
    for (int i = 0; i < numBars; i++) {
      ...
      if (barH > 0) {
        ...
        if (barH > 70) {
          color = 0xF81F; // Hồng đỏ
        } else if (barH > 40) {
          color = 0xFE60; // Vàng neon
        }
  ...
  if (canvas) {
    _tft.drawRGBBitmap(0, 32, canvas->getBuffer(), 240, 115);
  }
}