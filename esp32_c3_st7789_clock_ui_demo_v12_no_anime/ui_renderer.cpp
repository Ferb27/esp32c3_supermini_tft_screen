...
// Helper: vẽ đường thẳng siêu dày
...
void UIRenderer::updateBongoAnimation(int frameState) {
  ...
  int catX = 120;
  int catY = 85; 
  int tableY = 120; // Mặt bàn
  
  // --- MẶT BÀN ---
  _tft.fillRect(0, tableY, 240, 147 - tableY, 0x2124); // Màu xám đen
  _tft.drawLine(0, tableY, 240, tableY, 0xFFFF); // Cạnh bàn trắng
  
  // --- BÀN PHÍM ---
  int kbX = 60, kbY = tableY - 10, kbW = 120, kbH = 30;
  // Bàn phím nghiêng 1 chút
  _tft.fillRoundRect(kbX, kbY, kbW, kbH, 4, 0xD6BA); // Xám nhạt
  _tft.drawRoundRect(kbX, kbY, kbW, kbH, 4, 0x0000); // Viền đen
  // Phím bấm
  _tft.fillRect(kbX + 5, kbY + 5, kbW - 10, 12, 0xFFFF);
  _tft.drawRect(kbX + 5, kbY + 5, kbW - 10, 12, 0x8410);
  for(int i=1; i<10; i++) {
    _tft.drawLine(kbX + 5 + i*11, kbY + 5, kbX + 5 + i*11, kbY + 16, 0x8410);
  }
  // Spacebar
  _tft.fillRect(kbX + 30, kbY + 20, 60, 6, 0xFFFF);
  _tft.drawRect(kbX + 30, kbY + 20, 60, 6, 0x8410);
  
  // --- CƠ THỂ MÈO ---
  // Tai trái
  _tft.fillTriangle(catX - 35, catY - 10, catX - 15, catY - 35, catX - 5, catY - 15, 0xFFFF);
  _tft.drawTriangle(catX - 35, catY - 10, catX - 15, catY - 35, catX - 5, catY - 15, 0x0000);
  // Tai phải
  _tft.fillTriangle(catX + 35, catY - 10, catX + 15, catY - 35, catX + 5, catY - 15, 0xFFFF);
  _tft.drawTriangle(catX + 35, catY - 10, catX + 15, catY - 35, catX + 5, catY - 15, 0x0000);
  
  // Thân hình tròn mập mạp
  _tft.fillCircle(catX, catY, 35, 0x0000);
  _tft.fillCircle(catX, catY, 33, 0xFFFF);
  // Điền phần nửa dưới thành hình chữ nhật xuống bàn (để che viền tròn bên dưới)
  _tft.fillRect(catX - 35, catY, 71, tableY - catY, 0x0000);
  _tft.fillRect(catX - 33, catY, 67, tableY - catY, 0xFFFF);
  // Vẽ lại 2 đường viền 2 bên
  _tft.drawLine(catX - 35, catY, catX - 35, tableY, 0x0000);
  _tft.drawLine(catX + 35, catY, catX + 35, tableY, 0x0000);
  
  // Khuôn mặt
  // 2 mắt đen to
  _tft.fillCircle(catX - 14, catY - 5, 3, 0x0000);
  _tft.fillCircle(catX + 14, catY - 5, 3, 0x0000);
  // Miệng nhỏ
  _tft.drawLine(catX - 4, catY + 3, catX - 2, catY + 5, 0x0000);
  _tft.drawLine(catX - 2, catY + 5, catX, catY + 3, 0x0000);
  _tft.drawLine(catX, catY + 3, catX + 2, catY + 5, 0x0000);
  _tft.drawLine(catX + 2, catY + 5, catX + 4, catY + 3, 0x0000);
  
  // --- TAY MÈO ---
  bool leftOnKB  = (frameState == 0 || frameState == 1 || frameState == 3);
  bool rightOnKB = (frameState == 0 || frameState == 2 || frameState == 3);
  ...
  if (leftOnKB) {
    drawThickLine(_tft, lStartX, lStartY, lPawX, lPawY, 6, 0x0000);
    drawThickLine(_tft, lStartX+2, lStartY, lPawX, lPawY, 4, 0xFFFF);
  } else {
    // Tay giơ lên
    drawThickLine(_tft, lStartX, lStartY, catX - 45, catY - 10, 6, 0x0000);
    drawThickLine(_tft, lStartX+2, lStartY, catX - 45, catY - 10, 4, 0xFFFF);
  }
  
  int rStartX = catX + 36;
  int rStartY = catY + 15;
  int rPawX = kbX + kbW - 15;
  int rPawY = kbY + 10;
  
  if (rightOnKB) {
    drawThickLine(_tft, rStartX, rStartY, rPawX, rPawY, 6, 0x0000);
    drawThickLine(_tft, rStartX-2, rStartY, rPawX, rPawY, 4, 0xFFFF);
  } else {
    // Tay giơ lên
    drawThickLine(_tft, rStartX, rStartY, catX + 45, catY - 10, 6, 0x0000);
    drawThickLine(_tft, rStartX-2, rStartY, catX + 45, catY - 10, 4, 0xFFFF);
  }
}
