...
void IconManager::drawWeatherIcon(Adafruit_ST7789& tft, int x, int y, int weatherCode, int hour) {
  ...
  if (isBitmapEmpty(selectedIcon, 1024)) {
    ...
    // Vẽ các hình vector dự phòng sắc nét và thẩm mỹ cao (kích thước 40x40)
    if (weatherCode >= 200 && weatherCode < 300) {
      // Storm: Đám mây xám sẫm + Sấm sét vàng zig-zag lớn ở trung tâm
      uint16_t cloudColor = 0x5AEB;
      drawVectorCloud(tft, x, y - 2, cloudColor);
      
      uint16_t boltColor = 0xFE60; // Cyber Yellow
      ...
      // Rain: Đám mây xám xanh + Mưa rơi cyan rơi song song nghiêng góc
      uint16_t cloudColor = 0x7BEF;
      drawVectorCloud(tft, x, y - 2, cloudColor);
      
      uint16_t rainColor = 0x07FF; // Cyan
      ...
      // Cloud: Đám mây bồng bềnh
      if (isNight) {
        // Vẽ đám mây đêm với Mặt trăng siêu lớn và đám mây xám nhỏ phía trước (dễ nhận diện từ xa)
        
        // 1. Vẽ Mặt trăng khuyết khổng lồ
        uint16_t moonColor = 0xFFE0; // Vàng tươi độ tương phản cực cao
        tft.fillCircle(x + 22, y + 17, 14, moonColor);
        tft.fillCircle(x + 14, y + 12, 14, 0x0000); // Lõm trăng cắt góc màu đen

        // 2. Vẽ đám mây màu xám nhỏ phía trước góc dưới bên trái
        uint16_t cloudColor = 0x7BEF; // Màu xám trung tính dễ nhìn
        ...
        tft.drawFastHLine(x + 9, y + 35, 10, 0x5AEB); // Bóng đổ đám mây xám sẫm
        
        // 3. Vẽ các ngôi sao vàng lấp lánh ở nền xung quanh trăng
        uint16_t starColor = 0xFEE0; // Yellow
        
        // Ngôi sao 4 cánh bên trái
        tft.drawPixel(x + 5, y + 16, 0xFFFF); // Nhân trắng
        ...
        // Sao nhỏ phát xạ ở góc trên bên trái
        ...
        // Mặt trời lấp ló sau mây
        // 1. Vẽ mặt trời phía sau trước
        uint16_t sunColor = 0xFE60;
        tft.fillCircle(x + 28, y + 13, 7, sunColor);
        tft.fillCircle(x + 28, y + 13, 3, 0xFFFF); // Lõi trắng phát sáng
        tft.drawLine(x + 28, y + 4,  x + 28, y + 2,  sunColor);
        tft.drawLine(x + 37, y + 13, x + 39, y + 13, sunColor);
        tft.drawLine(x + 34, y + 7,  x + 36, y + 5,  sunColor);
        
        // 2. Vẽ đám mây trắng chồng lên phía trước
        uint16_t cloudColor = 0xFFFF; // Trắng tinh tế
      ...
      if (isNight) {
        // Moon: Trăng khuyết màu vàng-gold + các vì sao nhỏ lấp lánh xung quanh
        uint16_t moonColor = 0xFEE0;
        tft.fillCircle(x + 20, y + 20, 11, moonColor);
        tft.fillCircle(x + 15, y + 17, 11, 0x0000); // Lõm trăng
        ...
        // Sun: Mặt trời tròn to màu cam-vàng, lõi trắng sáng 3D với 8 tia nắng đối xứng xung quanh
        uint16_t sunColor = 0xFE60;
        tft.fillCircle(x + 20, y + 20, 9, sunColor);
        tft.fillCircle(x + 20, y + 20, 4, 0xFFFF); // Lõi sáng 3D
        for (int i = 0; i < 8; i++) {
          float a = i * 0.785398f;
    ...
    // Tránh đọc tràn mảng: Bitmap trong icons.cpp là 32x32 (1024), ta căn giữa nó trong ô 40x40
  ...
  if (selectedIcon != nullptr) {
    ...
    if (isBitmapEmpty(selectedIcon, 1024)) {
      if (type == ICON_TYPE_TEMP) {
        if (value < 22) {
          // COLD: Nhiệt kế màu xanh biển, cột thủy ngân thấp, vẽ bông tuyết bên phải
          uint16_t coldColor = 0x07FF; // Ocean Blue
          tft.drawRoundRect(x + 10, y + 2, 8, 22, 4, coldColor);
          tft.drawCircle(x + 14, y + 24, 7, coldColor);
          tft.fillCircle(x + 14, y + 24, 5, coldColor);
          tft.fillRect(x + 12, y + 16, 4, 4, coldColor); // Hạ thanh nhiệt kế xuống thấp
          
          // Vẽ bông tuyết bên cạnh
          ...
          // HOT: Nhiệt kế màu đỏ, cột thủy ngân cao kịch trần, vẽ ngọn lửa bên phải
          uint16_t hotColor = 0xF800; // Red
          tft.drawRoundRect(x + 10, y + 2, 8, 22, 4, 0xFFFF); // Khung màu trắng
          tft.drawCircle(x + 14, y + 24, 7, 0xFFFF);
          tft.fillCircle(x + 14, y + 24, 5, hotColor);
          tft.fillRect(x + 12, y + 6, 4, 14, hotColor); // Thanh nhiệt kế cao
          
          // Vẽ ngọn lửa bên cạnh
          tft.fillTriangle(x + 21, y + 20, x + 27, y + 20, x + 24, y + 6, 0xF800); // Lửa ngoài đỏ
          tft.fillTriangle(x + 22, y + 20, x + 26, y + 20, x + 24, y + 11, 0xFE60); // Nhân lửa vàng
        }
        else {
          // NORMAL: Nhiệt kế màu đỏ, cột thủy ngân ở mức giữa, không vẽ thêm gì
          uint16_t fluidColor = 0xF800; // Red
          tft.drawRoundRect(x + 10, y + 2, 8, 22, 4, 0xFFFF); // Khung màu trắng
          tft.drawCircle(x + 14, y + 24, 7, 0xFFFF);
          tft.fillCircle(x + 14, y + 24, 5, fluidColor);
          tft.fillRect(x + 12, y + 11, 4, 9, fluidColor); // Thanh nhiệt kế ở giữa
        }
